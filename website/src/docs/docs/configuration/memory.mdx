...
shuvmaki gives the agent a **persistent memory** through a `MEMORY.md` file in your project root. The agent reads it at session start and can update it, so hard-won context survives across sessions.
...
shuvmaki also injects small **awareness reminders** into a session when relevant, so the agent stays oriented as things change mid-session:
