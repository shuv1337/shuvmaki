...
shuvmaki surfaces OpenCode's **permission system** in Discord. When the agent tries something that needs approval, you get buttons in the thread. You can also pre-configure defaults and restrict individual sessions.

## Approval buttons

When the agent attempts a tool call that requires permission (a shell command, accessing files outside the project, etc.), shuvmaki posts three buttons in the thread:
...
To protect specific folders, add `deny` or `ask` rules to your `opencode.json`. Your project config is merged **after** shuvmaki's defaults and the last matching rule wins, so your rules take priority:
...
shuvmaki writes `"external_directory": { "*": "allow" }`, so the keys above merge on top of that wildcard. Paths you don't list stay allowed.
