...
description: How shuvmaki maps Discord channels to projects and threads to coding sessions, the core orchestration model.
icon: lucide:folder-tree
---

shuvmaki's whole design rests on one mapping: a Discord **channel is a project**, and a **thread is a coding session**. Understanding this makes everything else obvious.
...
shuvmaki partitions work along the two axes that actually matter:
...
When you add a project, shuvmaki creates a Discord channel and stores the link between that channel and a directory on your machine. The mapping lives in the channel topic as metadata:
...
A single shuvmaki instance runs on one machine and can serve **many channels across many Discord servers**. Each channel points at a directory on the machine where the bot runs. To control more machines, run shuvmaki on each and add their channels to the same server. See [Advanced Setup](../guides/advanced-setup) for multi-machine and multi-server layouts.
