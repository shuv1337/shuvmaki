...
Every thread is a session, and shuvmaki gives you a full set of commands to manage their lifecycle: pick up old ones, branch off, share, and clean up context.
...
- **`/session-id`** — show the session ID and a `kimaki attach` command to join it from your terminal.
...
shuvmaki also shows context usage automatically in the thread at roughly 10% increments (prefixed `⬦`), and at the end of each assistant turn in a footer like `kimakivoice ⋅ main ⋅ 2m 30s ⋅ 71% ⋅ claude-opus`.
