...
shuvmaki handles images in **both directions**: attach images to your message for the agent to look at, and see images the agent produces rendered inline in the thread.

## Sending images to the agent

Attach one or more images (or PDFs) to a Discord message. shuvmaki downloads each one, resizes it if needed, and sends it to the session as inline content the model can actually see.
