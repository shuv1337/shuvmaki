...
shuvmaki registers your project's **OpenCode commands, skills, and MCP prompts** as Discord slash commands. Anything you can invoke in OpenCode, you can invoke from Discord.

## How names map

shuvmaki adds a suffix so each kind of command is distinct and avoids colliding with shuvmaki's own commands:
...
After adding or changing OpenCode commands, skills, or MCP servers, **restart shuvmaki** so it re-registers the Discord slash commands. The `/restart-opencode-server` command also re-registers commands without a full restart.
