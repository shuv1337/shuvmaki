...
The **queue** lets you line up a message to be delivered **when the current run finishes**, instead of interrupting it. Plain OpenCode has no way to do this; shuvmaki tracks the queue itself.

## Why queue instead of just sending

A normal message during a run [acts as an interrupt](../core-concepts/message-handling): after ~3 seconds shuvmaki aborts the current step to deliver it. The queue is the opposite: it **waits politely** for the run to complete, then sends.
...
If the session is **busy**, the message waits and shuvmaki replies with its queue position. If the session is already **idle**, it dispatches immediately.
...
Once the run finishes, shuvmaki dispatches the queued message and shows it in the thread as:
