...
shuvmaki can run a prompt **on a schedule**: once at a future time, or repeatedly on a cron expression. This turns the bot into an autonomous worker that wakes up, does a job, and posts the result as a Discord thread you can reply to.
...
An exit code of **0 starts the session**. shuvmaki appends the command's stdout to the task prompt. Any other exit code skips that occurrence. Stdout and stderr are also written to the shuvmaki log.

Scheduled tasks **do not overlap by default**. If one occurrence still has an active OpenCode session, shuvmaki skips the next occurrence and checks again at the following scheduled time. Add `--allow-concurrency` only when parallel runs are safe.
...
kimaki task edit <id> --prompt "new prompt" [--send-at "new schedule"] \
  [--pre-run "command"] [--allow-concurrency true|false]
