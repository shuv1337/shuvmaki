...
- **From a project channel**, pass a `name`. shuvmaki creates a thread immediately (so you can start typing) and builds the worktree in the background. The branch is named `opencode/kimaki-<name>` and the thread is prefixed with `⬦ worktree:`.
- **From an existing thread**, the `name` is optional; shuvmaki derives it from the thread name.
...
- On success the `⬦ worktree:` prefix is removed from the thread title and shuvmaki reports the merged commit count.

## Conflict resolution by the agent

If the rebase hits **conflicts**, shuvmaki does not give up. It asks the AI model **in the thread** to resolve them: understand both sides, edit the conflicted files, `git add`, and `git rebase --continue`, repeating until the rebase finishes. Once the agent is done, run `/merge-worktree` again to complete the merge.
...
Run **`/worktrees`** to see every worktree for the channel's project (shuvmaki, OpenCode, or manual). The same view has a **Turn on / Turn off** button for automatic worktree creation on new sessions in that channel.

<Aside>
<Note>
Worktrees created by shuvmaki, by OpenCode, or manually all show up in `/worktrees` for the channel's project.
</Note>
</Aside>
