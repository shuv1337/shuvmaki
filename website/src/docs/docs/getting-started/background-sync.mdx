...
description: Automatically mirror OpenCode sessions started outside shuvmaki into Discord threads.
icon: refresh-cw
---

shuvmaki runs a background sync loop that mirrors **OpenCode sessions started outside of Discord** into Discord threads. If you start a session from the OpenCode CLI, TUI, or any other client, it automatically appears as a thread in the matching project channel.
...
- **Team collaboration.** If your team shares a machine running shuvmaki (a dev server, shared workstation, or remote box), sessions started by anyone on that machine show up in the shared Discord channel. Others can see what's happening, jump into the thread, or continue the work.
...
2. Skips sessions already managed by Discord (shuvmaki-originated sessions have a `<discord-user />` tag in their messages)
...
**One server for everything.** shuvmaki runs a single shared OpenCode server process for all projects. The sync loop reuses this same server, so there's no extra overhead.
...
shuvmaki still works normally for Discord-originated sessions; only the external session mirroring is turned off.
