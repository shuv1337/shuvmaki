...
description: Running multiple shuvmaki instances, multiple Discord servers, and architecture details.
icon: server
---
...
By default, shuvmaki stores its data in `~/.kimaki`. To run multiple bot instances on the same machine (e.g., for different teams or projects), use a separate `--data-dir` and optionally set `KIMAKI_LOCK_PORT` explicitly:
...
A single shuvmaki instance can serve multiple Discord servers. Install the bot in each server using the install URL shown during setup, then add project channels to each server.
...
To run shuvmaki on a remote machine instead of your laptop, put it in a container with a persistent volume. The first boot prints an install URL in the logs. See [Deploy shuvmaki on a VPS with Docker](./docker.mdx).
