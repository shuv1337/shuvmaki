---
title: CI & Automation
description: Start shuvmaki sessions from CI pipelines, cron jobs, or any automation. Includes GitHub Actions examples, scheduled tasks, and per-session permissions.
icon: workflow
---
...
If you're using the shared shuvmaki gateway bot (`kimaki --gateway`), the token is a **`clientId:clientSecret`** pair generated during onboarding. It looks like `a1b2c3d4-...:7f8e9d...`. This credential authenticates your kimaki instance against the gateway proxy; it is not a Discord bot token.
...
This workflow starts a shuvmaki session whenever a new issue is opened:
...
3. Make sure the shuvmaki bot is running on your machine
