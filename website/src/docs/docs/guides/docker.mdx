...
The container runs `kimaki --gateway`. On first boot Kimaki creates local credentials, prints an **install URL**, and waits for you to authorize the shared bot. After that it connects through the [gateway proxy](../reference/gateway-architecture.mdx) and listens for Discord messages.
...
The Kimaki log file is `/data/kimaki.log`. It resets every time the bot process starts. See [Troubleshooting](./troubleshooting.mdx) if sessions stall after the bot is online.

## One bot per machine

Each Kimaki process can only see directories **inside this container**. That is the same rule as a laptop install. To drive another machine, run another container (or a native `kimaki` process) there and install that instance into the same Discord server. See [Advanced Setup](./advanced-setup.mdx).
