...
shuvmaki supports any OpenCode plugin. Create this file at `~/.config/opencode/plugins/abort-on-x.ts`:
...
shuvmaki runs on OpenCode, so any OpenCode plugin works out of the box. You can:
