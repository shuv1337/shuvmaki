...
shuvmaki's **gateway mode** lets multiple users share a single Discord bot instead of each user creating their own. A Rust proxy sits between Discord and every kimaki CLI instance, routing events to the right client based on guild authorization.
...
| **shuvmaki CLI** | Nothing (delegates to above layers) | N/A | N/A |
